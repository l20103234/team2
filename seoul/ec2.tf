...
resource "aws_instance" "k8s-argocd-Server" {
  ...
  tags = {
    Name = "k8s-argocd-Server"
  }
}

# Private에 k8s-dms Server 생성
resource "aws_instance" "k8s-dms-Server" {
  ami                    = coalesce(data.aws_ami.amzlinux2.id, var.image_id)
  instance_type          = "t3.medium"
  key_name               = "yoowonjongkey"
  vpc_security_group_ids = [aws_security_group.dms_security.id]
  subnet_id              = aws_subnet.pri_sub_c.id
...
resource "aws_instance" "k8s-dev-argocd-Server" {
  ...
  tags = {
    Name = "k8s-dev-argocd-Server"
  }
}

# Private에 k8s-dms Server 생성
resource "aws_instance" "k8s-dev-dms-Server" {
  ami                    = coalesce(data.aws_ami.amzlinux2.id, var.image_id)
  instance_type          = "t3.medium"
  key_name               = "yoowonjongkey"
  vpc_security_group_ids = [aws_security_group.dev_dms_security.id]
  subnet_id              = aws_subnet.dev_pri_sub_c.id
